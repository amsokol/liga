...
# This script freezes zero in MD_IO, THEN adds ligatures
...
INPUT_DIR="$LIGA_DIR/input/MD IO"
FROZEN_INPUT_DIR="$LIGA_DIR/input/MD IO Frozen ZERO"
OUTPUT_NAME="MD IO"
OUTPUT_DIR="$LIGA_DIR/output/$OUTPUT_NAME"
CONFIG="$SRC/config.py"

echo "Step 1: Freezing zero in original fonts..."
...
for font_file in "$INPUT_DIR"/*.ttf; do
    if [ -f "$font_file" ]; then
        ...
        echo "  Freezing zero in $filename..."
        python3 "$LIGA_DIR/freeze_ss01.py" "$font_file" "$FROZEN_INPUT_DIR/$filename" zero
    fi
done
...
# Copy license
# cp "$LIGA_DIR/input/MD_IO/OFL.txt" "$OUTPUT_DIR" 2>/dev/null || true

echo ""
echo "✨ All done! Fonts with frozen zero + ligatures are in: $OUTPUT_DIR"
